...
class Register implements Observable
{
        ...
        try {
            ...
            if ($this->checkToken($serverToken)) {
                ...
                if ($this->processStatus === NULL)
                {
                    ...
                    $this->user = $userManager->return();

                        // insert new user
                    if(!$this->repository->insert($this->user))
                    {
    ...
    public function getUser(bool $flag = FALSE): User{
        if($flag)
            $this->user = $this->repository->fetchByNick($this->user->getNick());
